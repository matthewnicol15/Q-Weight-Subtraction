...
// Macro name
void Strangeness_2_RGA_Q_Weight(){
  ...
  Input_File_Name<<"/mnt/f/PhD/Trees/Dibaryon/RGA/Strangeness_2/RGA_SPRING2019_Inbending_skim4_Exclusive_S2_Tree_020721_01.root";
  Input_TTree_Name<<"RGA_Skim4_Tree_020721_01";
  // Input_TTree_Name<<"Energy_"<<Energy_Bin;

  Output_File_Name<<"/mnt/f/PhD/Trees/Dibaryon/RGA/Strangeness_2/RGA_Spring2019_Inbending_skim4_Exclusive_Tree_Friend_020721_"<<Energy_Bin<<".root";
  ...
  Double_t mass_kp, mass_kp_other_1;
  ...
  auto* hinv_lambda_1=new TH1F("hinv_lambda_1","Invariant mass of p #pi^{-};M(p #pi^{-}) [GeV];Counts",500,1,2);
  auto* hinv_lambda_2=new TH1F("hinv_lambda_2","Invariant mass of p #pi^{-};M(p #pi^{-}) [GeV];Counts",500,1,2);
  ...
  auto* hmass_kp_2=new TH1F("hmass_kp_2","K^{+} mass;M(K^{+});Counts",100,0.2,0.8);
  ...
  auto* h_delta_beta_kp_1=new TH2F("h_delta_beta_kp_1","",200,0,11,200,-1,1);
  auto* h_delta_beta_kp_2=new TH2F("h_delta_beta_kp_2","",200,0,11,200,-1,1);
  auto* h_delta_beta_pr=new TH2F("h_delta_beta_pr","",200,0,11,200,-1,1);
  auto* h_delta_beta_pim_1=new TH2F("h_delta_beta_pim_1","",200,0,11,200,-1,1);
  auto* h_delta_beta_pim_2=new TH2F("h_delta_beta_pim_2","",200,0,11,200,-1,1);
  ...
  TLorentzVector lambda_1, lambda_2, Lambda_other;
  ...
  for(Long64_t i = 0; i < nentries; i++){
    ...
    if(v_kp.size()==2 && v_el.size()==1 && v_pr.size()==1 && v_pim.size()==2){
      ...
      if(v_mass_kp.at(0) > 0.65){
        ...
        // Missing mass of e' K^{+} K^{+}, looking for cascade ground state
        miss1 = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1);
        // Missing mass of e' K^{+} K^{+} pi^{-} pi^{-}, looking for p
        miss2 = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_pim.at(0) - v_pim.at(1);
        // Invariant mass of p pi^{-}, looking for lambda ground state
        lambda_1 = v_pr.at(0) + v_pim.at(0);
        lambda_2 = v_pr.at(0) + v_pim.at(1);
        // Missing mass of all detected particles, should have peak at 0
        missall = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el.at(0) - v_kp.at(0) - v_kp.at(1) - v_pim.at(0) - v_pim.at(1) - v_pr.at(0);
        // Determining the photon TLorentzVector
        photon = (TLorentzVector)*readbeam - v_el.at(0);
        // COM used for boosting
        COM = v_el.at(0) + v_kp.at(0) + v_kp.at(1) + v_pr.at(0) + v_pim.at(0) + v_pim.at(1);
        ...
        h_delta_beta_pim_2->Fill(v_pim.at(1).Rho(),v_delta_beta_pim.at(1));
        ...
        hinv_lambda_1->Fill(lambda_1.M());
        hinv_lambda_2->Fill(lambda_2.M());
        ...
        for(Long64_t m = 0; m < 1; m++){
          ...
          if(v_el_other.size() == 1 && v_kp_other.size() == 2 && v_pr_other.size() == 1 && v_pim_other.size() == 2){

            // Missing mass of e' K^{+}, looking for lambda ground state
            miss1_other = (TLorentzVector)*readbeam + (TLorentzVector)*readtarget - v_el_other.at(0) - v_kp_other.at(0) - v_kp_other.at(1);
            ...
            COM_other = v_el_other.at(0) + v_kp_other.at(0) + v_kp_other.at(1) + v_pim_other.at(0) + v_pim_other.at(1) + v_pr_other.at(0);

            mass_kp_other_1 = sqrt((pow(v_kp_other.at(0).Rho(),2) / (pow(beta_tof_kp_other,2))) - pow(v_kp_other.at(0).Rho(),2));

            // cout<<v_kp_other.size()<<" "<<v_beta->size()<<endl;
            if(mass_kp_other_1 > 0.65)continue;
            ...
            if(Nearest_Neighbour_Size<1){
              ...
              v_NN_mKp.push_back(mass_kp_other_1);
            }
            // If there are entries
              ...
              for(Int_t k = Nearest_Neighbour_Size - 1; k >= 0; k --){
                ...
                if(distance < v_NN_d.at(0)){
                  ...
                  v_NN_mKp.insert(itPos2MM, mass_kp_other_1);
                ...
                if(distance > v_NN_d.at(k)){
                  ...
                  v_NN_mKp.insert(itPosMM, mass_kp_other_1);
      ...
      if(lambda_1.M() < 1.14 || lambda_2.M() < 1.14){
